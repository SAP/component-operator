...
"$GOBIN"/client-gen \
  ...
  --output-pkg github.com/sap/component-operator/pkg/client/clientset \
  --output-dir "$TEMPDIR"/pkg/client/clientset \
  --plural-exceptions Component:components

"$GOBIN"/lister-gen \
  --go-header-file "$BASEDIR"/hack/boilerplate.go.txt \
  --output-pkg github.com/sap/component-operator/pkg/client/listers \
  --output-dir "$TEMPDIR"/pkg/client/listers \
  --plural-exceptions Component:components \
  github.com/sap/component-operator/tmp/gen/apis/core.cs.sap.com/v1alpha1
...
"$GOBIN"/informer-gen \
  ...
  --go-header-file "$BASEDIR"/hack/boilerplate.go.txt \
  --output-pkg github.com/sap/component-operator/pkg/client/informers \
